...
# provo el Padé-Weierstrass and formulaicó nova
...
#p=prima, '
Up = np.zeros((prof, npqpv), dtype=complex) # matriu on es guarden els coeficients de les tensions no slack primes
Up_re = np.zeros((prof, npqpv), dtype=float)
Up_im = np.zeros((prof, npqpv), dtype=float)
Xp = np.zeros((prof, npqpv), dtype=complex)
Xp_re = np.zeros((prof, npqpv), dtype=float)
Xp_im = np.zeros((prof, npqpv), dtype=float)
Qp = np.zeros((prof, npqpv), dtype=complex)
Upw = np.zeros((prof, nsl), dtype=complex)  # matriu on es guarden els coeficients de les tensions slack primes
...
for i in range(n):
    if i not in sl:  # per la fila de l'slack no s'ha de fer
        for j in range(n):
...
print('suma', np.sum(Ybtilde, axis=1))
...
MAT = vstack((hstack((G + gamma_re, -B, XIM)),
              hstack((B + gamma_im, G, XRE)),
              hstack((VRE, VIM, EMPTY))), format='csc')
...
print(abs(Upfi * Us0[pqpv]))
print(angle(Upfi * Us0[pqpv]))
