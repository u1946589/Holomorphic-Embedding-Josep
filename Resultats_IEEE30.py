...
# IGUAL QUE PW_full però he tocat una mica els índexs dels slacks del P-W
# P-W fa millorar, convergeix més ràpid.
...
df_top = pd.read_excel('IEEE30.xlsx', sheet_name='Topologia')  # DataFrame of the topology
df_bus = pd.read_excel('IEEE30.xlsx', sheet_name='Busos')  # Dataframe of the buses
...
limit = 12  # límit per tal que recurrents no tirin error. Si pocs busos, millor límit baix
if limit > prof:
    limit = prof - 1
Ux = np.copy(U)
...
#RHO
Uf1 = np.zeros((prof, npqpv), dtype=complex)
Uf1[:, :] = U[:, :]
for i in range(npqpv):
    U_rho[i] = rho(Uf1[:, i], limit)
U_rho[pqpv] = U_rho[pqpv_]
U_rho[sl] = np.nan
#FI RHO

#THETA
Uf2 = np.zeros((prof, npqpv), dtype=complex)
Uf2[:, :] = U[:, :]
for i in range(npqpv):
    U_theta[i] = theta(Uf2[:, i], limit)
U_theta[pqpv] = U_theta[pqpv_]
U_theta[sl] = np.nan
#FI THETA

#ETA
Uf3 = np.zeros((prof, npqpv), dtype=complex)
Uf3[:, :] = U[:, :]
for i in range(npqpv):
    U_eta[i] = eta(Uf3[:, i], limit)
...
Us0[sl] = 1 + s0 * V_sl[nsl_counted[sl]-1] - s0 * 1  # he canviat una mica els índexs de l'slack...
#print(Us0[sl])
Upw[0] = 1
Upw[1, nsl_counted[sl]-1] = (1-s0)*(V_sl[:]-1)/(1+s0*(V_sl[:]-1))  # V prima de l'slack amb tots els coef, quasi tots són 0
...
Ybtilde[:, :] += s0 * Yahat[:, :]
Yahat[:, :] = (1 - s0) * Yahat[:, :]  # serà la que s'incrustarà amb s'
...
"""
if npq > 0:  # per agafar el que he modificicat a Yb!! ho trec d'una banda i li poso a l'altra. NO, es cancel·len!!?
...
#print('Yahatred',Yahatred)
#print('Yahatw',Yahatw)
#print('Ybtildered',Ybtildered)
#print('Ybtildew',Ybtildew)
...
Amm = abs(MAT.todense())
#A = abs(np.real(np.linalg.inv((MAT.todense()))))  # la inversa, no surt gaire interessant
figure(1)
#imshow(A, interpolation='nearest', cmap=plt.get_cmap('binary'))
imshow(Amm, interpolation='nearest', cmap=plt.get_cmap('gist_heat'))
plt.gray()
plt.show()


plt.spy(Amm)  # o blanc o negre
plt.show()

Bmm = coo_matrix(MAT)  # passar a matriu dispersa altra vegada per calcular densitat
density = Bmm.getnnz() / np.prod(Bmm.shape) * 100  # percentual
