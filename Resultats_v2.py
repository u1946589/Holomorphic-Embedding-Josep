...
df_top = pd.read_excel('Pegase2869.xlsx', sheet_name='Topologia')  # DataFrame of the topology
df_bus = pd.read_excel('Pegase2869.xlsx', sheet_name='Busos')  # Dataframe of the buses
...
for i in range(nl):
    df_top.iloc[i, 5] = df_top.iloc[i, 5] * (np.cos(df_top.iloc[i, 6]) + np.sin(df_top.iloc[i, 6]) * 1j)
...
for i in range(nl):
    if df_top.iloc[i, 5] != 1:
        ...
        Ytap[df_top.iloc[i, 0], df_top.iloc[i, 0]] += 1 / (df_top.iloc[i, 2] + df_top.iloc[i, 3] * 1j) / (
                    abs(df_top.iloc[i, 5]) ** 2) - 1 / (df_top.iloc[i, 2] + df_top.iloc[i, 3] * 1j)
...
for i in range(n):  # de la sheet busos
    ...
    Yshunts[df_bus.iloc[i, 0]] += df_bus.iloc[i, 6] + df_bus.iloc[i, 7] * 1j  # també la part real
