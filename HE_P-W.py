...
plt.plot(vec_1n[50:len(U)], abs(bb[50:len(U), 29]), 'ro', markersize=2)  # a partir de 50 per no agafar els primers
...
s0 = 1  # s0 = 0.9 com podria ser un altre valor... veure que convé més
...
Up = np.zeros((prof, npqpv), dtype=complex)  # voltatges nous, V'
Up_re = np.zeros((prof, npqpv), dtype=complex)
Up_im = np.zeros((prof, npqpv), dtype=complex)
Xp = np.zeros((prof, npqpv), dtype=complex)
Xp_re = np.zeros((prof, npqpv), dtype=complex)
Xp_im = np.zeros((prof, npqpv), dtype=complex)
Qp = np.zeros((prof, npqpv), dtype=complex)  # unknown reactive powers
...
if npv > 0:
    ...
    RHS = np.r_[valor.real,
                valor.imag,
                vec_W[pv_] / abs(U_s0[pv_]) ** 2 - np.real(Up[0, pv_] * np.conj(Up[0, pv_]))]
...
print(Up[prof - 1, :])  # no sembla que hagi convergit molt ràpid... passar-ho per Padé?


Upf = sum(Up[:, range_pqpv])
print(abs(U_s0 * Upf))

Upfpade = pade4all(prof - 1, Up, 1)
print(Upfpade)
